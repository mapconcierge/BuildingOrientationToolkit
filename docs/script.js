...
function setupMap() {
  ...
  map.on("load", () => {
    ...
    map.addLayer({
      ...
      source: "buildings",
      paint: {
        "fill-color": ["interpolate", ["linear"], ["get", "roundness"], 0, "#be123c", 0.5, "#f97316", 1, "#22c55e"],
        "fill-opacity": 0.55,
      },
    });
  ...
  features.forEach((feature) => {
    Object.keys(feature.properties || {}).forEach((key) => {
      if (!key.startsWith("__") &&
          !["area_sqm", "perimeter_m", "roundness", "centroid_lat", "centroid_lon", "orientation_deg"].includes(key)) {
